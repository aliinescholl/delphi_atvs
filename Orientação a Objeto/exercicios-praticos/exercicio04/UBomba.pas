...
begin
  if QuantidadeCombustivel < aLitrosAbastecidos then
   ...
   if aLitrosAbastecidos <= 0 then
    raise Exception.Create('Não é póssível abastecer valores menor que 0!');
...
begin
  result := 'Há ' + FormatFloat('###,#00.00', QuantidadeCombustivel) + ' litros restantes ' + SlineBreak +
            'Preço do Litro: ' + CurrToSTr(ValorPorlitro);
end;

...
begin
  ...
  begin
    ...
    if xDiferenca = 0 then
      memo_listagem.Lines.Add('Não houve lucro vendendo R$' +
      ...
      memo_listagem.Lines.Add('Houve prejuízo de R$' + formatFloat('###,##0.00',
        xDiferenca) + ' vendendo a R$' + formatFloat('###,##0.00', xVendas.venda)
        + ' o que custou R$' + formatFloat('###,##0.00', xVendas.custo));
    memo_listagem.Lines.Add('');
    memo_listagem.Lines.Add('A média entre preço de custo e preço de venda foi de: R$ ' + FormatFloat('###,##0.00', xMedia));
  end;

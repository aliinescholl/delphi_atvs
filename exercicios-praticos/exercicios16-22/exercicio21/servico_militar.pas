...
begin
  ...
  begin
    ...
    xPessoa.saude := AnsiLowerCase(inputbox('Informar', 'Saúde(boa,media,ruim)):', ''));
    FPessoa[i] := xPessoa;
  end;
...
begin
  ...
  begin
    ...
    if ((xPessoa.idade >= 18) and (xPessoa.sexo = 'masculino')) and
      ...
      begin
        memo_resultado.Lines.Add(xPessoa.nome + ' ' + IntToStr(xPessoa.idade) + ' anos do sexo '+ xPessoa.sexo + ' e tem a saúde '+xPessoa.saude +
                                ' está apto para prestar o serviço militar');
        Inc(xcontAptos)
      end
      ...
      begin
      memo_resultado.Lines.Add(xPessoa.nome + ' ' + IntToStr(xPessoa.idade) + ' anos do sexo '+xPessoa.sexo + ' e tem a saúde ' +xPessoa.saude +
                                ' não está apto para prestar o serviço militar');
      Inc(xcontNAptos)
      end;

    end;
  memo_resultado.Lines.Add(' ');
  memo_resultado.Lines.Add('Há ' + IntToStr(xcontAptos) + ' aptos para o serviço militar e ' + SLineBreak
                            + IntToSTr(xcontNAptos) + ' não aptos para o serviço militar');
  memo_resultado.Lines.Add('São ' + IntTostr(xcontAptos+xcontNAptos) + ' pessoas no total');
end;

...
(* function Somar(var num1, num2 : Integer):Integer ;
  ...
  showmessage(IntToStr(resultado) + ' É o resultado.');
  end;

  func_while:
  begin
  resultado := Sangria(valorcaixa1, valorcaixa2);
  showmessage(IntToStr(resultado) + ' Dias você terá um saldo = 0')
  end;

  func_repeat:
  begin
  r := Meta(valorcaixa1, valorcaixa2);
  showmessage(IntToStr(r) + ' Dias você atingirá sua meta')
  end;
  end;
  end; *)
...
begin
  ...
  ShowMessage('Em ' + IntToStr(xContador) + 'Dias você terá zerado seu saldo');
...
begin
  ...
  ShowMessage('Em ' + IntToStr(xDias) + ' dias você terá atingido a sua meta.');

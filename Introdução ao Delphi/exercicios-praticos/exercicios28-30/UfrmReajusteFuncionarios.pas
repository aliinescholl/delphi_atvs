...
begin
  for i := 0 to Length(FFuncionario) do
    begin
      ...
      memo_resultado.Lines.Add('O funcionário ' + xFuncionario.nome + ' tinha um sálario de R$' + FloatToStr(xFuncionario.salario) +
                                ' e após o reajuste de ' + FormatFloat('###,##0.00', xFuncionario.reajuste) + ' passará a receber R$' +
                                FormatFloat('###,##0.00', xFuncionario.novoSalario))

    end;
    memo_resultado.Lines.Add(' folha da empresa aumentará em R$' + FormatFloat('###,##0.00', xAumentoFolha))
end;
